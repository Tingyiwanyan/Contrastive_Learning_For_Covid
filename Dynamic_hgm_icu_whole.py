...
class dynamic_hgm():
    ...
    def get_positive_patient(self, center_node_index):
        ...
        for j in range(self.time_sequence):
            ...
            if flag == 0:
                ...
                start_time = self.kg.dic_patient[center_node_index]['in_icu_hour'] - self.predict_window_prior + float(
        ...
        for i in range(self.positive_lab_size):
            ...
            for j in range(self.time_sequence):
                ...
                if flag == 0:
                    ...
                    start_time = self.kg.dic_patient[patient_id]['in_icu_hour'] - self.predict_window_prior + float(
                        j) * self.time_step_length
                    end_time = start_time + self.time_step_length
    ...
    def config_model_cl_rnn(self):
        ...
        self.SGNN_loss_contrast()
        #self.train_step_neg = tf.compat.v1.train.AdamOptimizer(1e-3).minimize(self.negative_sum)
        self.train_step_neg = tf.compat.v1.train.AdamOptimizer(1e-3).minimize(self.negative_sum+0.8*self.negative_sum_contrast)
    ...
    def test(self, data):
        ...
        """
        self.test_data_scores = self.test_att_score[1][self.correct_predict_icu, :, 0, :]
