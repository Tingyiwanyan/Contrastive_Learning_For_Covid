...
class LSTM_model():
    ...
    def __init__(self,kg,data_process):
        ...
        self.input_x_vital = tf.placeholder(tf.float32,[None,self.time_sequence,self.item_size])
        self.input_x_lab = tf.placeholder(tf.float32,[None,self.time_sequence,self.lab_size])
        ...
        """
        self.input_demo_ = tf.placeholder(tf.float32,[None,self.demo_size])
        self.input_x_com = tf.placeholder(tf.float32,[None,self.com_size])
    ...
    def get_batch_train_period(self,data_length,start_index,data):
        ...
        for i in range(data_length):
            ...
            for j in range(self.time_sequence):
                ...
                if flag == 0:
                    start_time = self.kg.dic_patient[self.patient_id]['discharge_hour'] - self.predict_window_prior + float(j)*self.time_step_length
                    end_time = start_time + self.time_step_length
                    self.check_start_time = start_time
                else:
                    start_time = self.kg.dic_patient[self.patient_id]['death_hour'] - self.predict_window_prior + float(j)*self.time_step_length
                    end_time = start_time + self.time_step_length
                    self.check_start_time = start_time
