    ...
    def gen_demo_spec_csv(self,data,name_to_store):
        ...
        for i in data:
            ...
            for j in self.demo_spec_each.keys():
                ...
                percent_90 = np.percentile(values_filtered, 70)
...
if __name__ == "__main__":
    ...
    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                       "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_24_RNN_CE_intubation_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                       "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_24_RNN_CE_intubation_full", index=False)

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_ce_rnn()
    dhgm.train()
    dhgm.test(dhgm.test_data_final)
    np.save("embedding_24h_rnn_ce_intubation_full.npy",dhgm.test_patient)
    np.save("logit_24h_rnn_ce_intubation_full.npy",dhgm.test_logit)
    ...
    print("now training 24h RNN with CL intubation_full")
    dhgm = dynamic_hgm(kg, process_data, 4)
    dhgm.cross_validation("cl_rnn")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_24_RNN_CL_intubation_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_24_RNN_CL_intubation_full", index=False)

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_cl_rnn()
    dhgm.train()
    dhgm.test(dhgm.test_data_final)
    np.save("embedding_24h_rnn_cl_intubation_full.npy", dhgm.test_patient)
    np.save("logit_24h_rnn_cl_intubation_full.npy", dhgm.test_logit)

    dhgm.sess.close()

    del dhgm
    
    print("now training 24h RETAIN with CE intubation_full")
    dhgm = dynamic_hgm(kg, process_data, 4)
    dhgm.cross_validation("ce_retain")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_24_RETAIN_CE_intubation_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_24_RETAIN_CE_intubation_full", index=False)

    dhgm.gen_heap_map_csv("heat_map_24h_retain_CE_intubation_full")

    dhgm.train_data = process_data.train_mortality
    dhgm.config_moldel_ce_retain()
    dhgm.train()
    dhgm.test_retain(dhgm.test_data_final)
    np.save("embedding_24h_retain_ce_intubation_full.npy", dhgm.test_patient)
    np.save("logit_24h_retain_ce_intubation_full.npy", dhgm.test_logit)

    dhgm.sess.close()

    del dhgm

    print("now training 24h RETAIN with CL intubation full")
    dhgm = dynamic_hgm(kg, process_data, 4)
    dhgm.cross_validation("cl_retain")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_24_RETAIN_CL_intubation_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_24_RETAIN_CL_intubation_full", index=False)

    dhgm.gen_heap_map_csv("heat_map_24h_retain_CL_intubation_full")

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_cl_retain()
    dhgm.train()
    dhgm.test_retain(dhgm.test_data_final)
    np.save("embedding_24h_retain_cl_intubation_full.npy", dhgm.test_patient)
    np.save("logit_24h_retain_cl_intubation_full.npy", dhgm.test_logit)
    ...
    dhgm = dynamic_hgm(kg, process_data, 8)
    print("now training 48h RNN with CE intubation full")
    dhgm.cross_validation("ce_rnn")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_48_RNN_CE_intubation_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_48_RNN_CE_intubation_full", index=False)

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_ce_rnn()
    dhgm.train()
    dhgm.test(dhgm.test_data_final)
    np.save("embedding_48h_rnn_ce_intubation_full.npy", dhgm.test_patient)
    np.save("logit_48h_rnn_ce_intubation_full.npy", dhgm.test_logit)

    dhgm.sess.close()

    del dhgm

    print("now training 48h RNN with CL intubation full")
    dhgm = dynamic_hgm(kg, process_data, 8)
    dhgm.cross_validation("cl_rnn")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_48_RNN_CL_intubation_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_48_RNN_CL_intubation_full", index=False)

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_cl_rnn()
    dhgm.train()
    dhgm.test(dhgm.test_data_final)
    np.save("embedding_48h_rnn_cl_intubation_full.npy", dhgm.test_patient)
    np.save("logit_48h_rnn_cl_intubation_full.npy", dhgm.test_logit)
    ...
    print("now training 48h RETAIN with CE intubation full")
    dhgm = dynamic_hgm(kg, process_data, 8)
    dhgm.cross_validation("ce_retain")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_48_RETAIN_CE_intubation_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_48_RETAIN_CE_intubation_full", index=False)
    ...
    dhgm.train()
    dhgm.test_retain(dhgm.test_data_final)
    np.save("embedding_48h_retain_ce_intubation_full.npy", dhgm.test_patient)
    np.save("logit_48h_retain_ce_intubation_full.npy", dhgm.test_logit)
    ...
    print("now training 48h RETAIN with CL intubation full")
    dhgm = dynamic_hgm(kg, process_data, 8)
    dhgm.cross_validation("cl_retain")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_48_RETAIN_CL_intubation_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_48_RETAIN_CL_intubation_full", index=False)
    ...
    dhgm.train()
    dhgm.test_retain(dhgm.test_data_final)
    np.save("embedding_48h_retain_cl_intubation_full.npy", dhgm.test_patient)
    np.save("logit_48h_retain_cl_intubation_full.npy", dhgm.test_logit)
    ...
    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_24_RNN_CE_icu_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_24_RNN_CE_icu_full", index=False)

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_ce_rnn()
    dhgm.train()
    dhgm.test(dhgm.test_data_final)
    np.save("embedding_24h_rnn_ce_icu_full.npy", dhgm.test_patient)
    np.save("logit_24h_rnn_ce_icu_full.npy", dhgm.test_logit)

    dhgm.sess.close()

    del dhgm

    print("now training 24h RNN with CL icu full")
    dhgm = dynamic_hgm(kg, process_data, 4)
    dhgm.cross_validation("cl_rnn")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_24_RNN_CL_icu_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_24_RNN_CL_icu_full", index=False)

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_cl_rnn()
    dhgm.train()
    dhgm.test(dhgm.test_data_final)
    np.save("embedding_24h_rnn_cl_icu_full.npy", dhgm.test_patient)
    np.save("logit_24h_rnn_cl_icu_full.npy", dhgm.test_logit)

    dhgm.sess.close()

    del dhgm

    print("now training 24h RETAIN with CE icu full")
    dhgm = dynamic_hgm(kg, process_data, 4)
    dhgm.cross_validation("ce_retain")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_24_RETAIN_CE_icu_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_24_RETAIN_CE_icu_full", index=False)

    dhgm.gen_heap_map_csv("heat_map_24h_retain_CE_icu_full")

    dhgm.train_data = process_data.train_mortality
    dhgm.config_moldel_ce_retain()
    dhgm.train()
    dhgm.test_retain(dhgm.test_data_final)
    np.save("embedding_24h_retain_ce_icu_full.npy", dhgm.test_patient)
    np.save("logit_24h_retain_ce_icu_full.npy", dhgm.test_logit)

    dhgm.sess.close()

    del dhgm

    print("now training 24h RETAIN with CL icu full")
    dhgm = dynamic_hgm(kg, process_data, 4)
    dhgm.cross_validation("cl_retain")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_24_RETAIN_CL_icu_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_24_RETAIN_CL_icu_full", index=False)

    dhgm.gen_heap_map_csv("heat_map_24h_retain_CL_icu_full")

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_cl_retain()
    dhgm.train()
    dhgm.test_retain(dhgm.test_data_final)
    np.save("embedding_24h_retain_cl_icu_full.npy", dhgm.test_patient)
    np.save("logit_24h_retain_cl_icu_full.npy", dhgm.test_logit)
    ...
    dhgm = dynamic_hgm(kg, process_data, 8)
    print("now training 48h RNN with CE icu full")
    dhgm.cross_validation("ce_rnn")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_48_RNN_CE_icu_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_48_RNN_CE_icu_full", index=False)

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_ce_rnn()
    dhgm.train()
    dhgm.test(dhgm.test_data_final)
    np.save("embedding_48h_rnn_ce_icu_full.npy", dhgm.test_patient)
    np.save("logit_48h_rnn_ce_icu_full.npy", dhgm.test_logit)

    dhgm.sess.close()

    del dhgm

    print("now training 48h RNN with CL icu full")
    dhgm = dynamic_hgm(kg, process_data, 8)
    dhgm.cross_validation("cl_rnn")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_48_RNN_CL_icu_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_48_RNN_CL_icu_full", index=False)

    dhgm.train_data = process_data.train_mortality
    dhgm.config_model_cl_rnn()
    dhgm.train()
    dhgm.test(dhgm.test_data_final)
    np.save("embedding_48h_rnn_cl_icu_full.npy", dhgm.test_patient)
    np.save("logit_48h_rnn_cl_icu_full.npy", dhgm.test_logit)

    dhgm.sess.close()

    del dhgm

    print("now training 48h RETAIN with CE icu full")
    dhgm = dynamic_hgm(kg, process_data, 8)
    dhgm.cross_validation("ce_retain")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_48_RETAIN_CE_icu_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_48_RETAIN_CE_icu_full", index=False)
    ...
    dhgm.train()
    dhgm.test_retain(dhgm.test_data_final)
    np.save("embedding_48h_retain_ce_icu_full.npy", dhgm.test_patient)
    np.save("logit_48h_retain_ce_icu_full.npy", dhgm.test_logit)

    dhgm.sess.close()

    del dhgm

    print("now training 48h RETAIN with CL icu full")
    dhgm = dynamic_hgm(kg, process_data, 8)
    dhgm.cross_validation("cl_retain")

    df_prc = pd.DataFrame({"recall_ave_seq": dhgm.recall_ave_score, "precision_ave_seq": dhgm.precision_ave_score,
                           "std_precision": dhgm.std_precision})
    df_prc.to_csv("pr_curve_48_RETAIN_CL_icu_full", index=False)

    df_roc = pd.DataFrame({"tp_ave_seq": dhgm.tp_ave_score, "fp_ave_seq": dhgm.fp_ave_score,
                           "std_tp": dhgm.std_tp})
    df_roc.to_csv("roc_curve_48_RETAIN_CL_icu_full", index=False)
    ...
    dhgm.train()
    dhgm.test_retain(dhgm.test_data_final)
    np.save("embedding_48h_retain_cl_icu_full.npy", dhgm.test_patient)
    np.save("logit_48h_retain_cl_icu_full.npy", dhgm.test_logit)
