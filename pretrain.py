...
class LSTM_model():
    ...
    def __init__(self, kg, data_process):
        ...
        self.weight_classification_w = tf.Variable(
            self.init_weight_classification_w(shape=(self.latent_dim, 1)))  # +self.latent_dim_demo,1)))
    ...
    def softmax_loss(self):
        ...
        self.output_layer = tf.math.sigmoid(
            tf.math.add(tf.matmul(self.hidden_last, self.weight_classification_w), self.bias_classification_b))
        ...
        for i in range(data_length):
            ...
            for j in range(self.time_sequence):
                ...
                if flag == 0:
                    ...
                    start_time = self.kg.dic_patient[self.patient_id]['death_hour'] - self.predict_window_prior + float(
    ...
    def get_batch_train_period_intubate(self, data_length, start_index, data):
        """
        get period train data
        """
        train_one_batch_vital = np.zeros((data_length, self.time_sequence, self.item_size))
    ...
    def train_intubate(self):
        ...
        for j in range(self.epoch):
            ...
            print(j)
            for i in range(iteration):
                self.train_one_batch, self.train_one_batch_lab, self.logit_one_batch, self.train_one_batch_demo, self.train_one_batch_com = self.get_batch_train_period_intubate(
                    self.batch_size, i * self.batch_size, self.train_data)
                self.err_ = self.sess.run(
                    [self.cross_entropy, self.train_step_cross_entropy, self.init_hiddenstate, self.output_layer],
                    feed_dict={self.input_x_vital: self.train_one_batch,
    ...
    def test_intubation(self, data):
        ...
        test_data, self.test_data_lab, self.test_logit, self.test_demo, self.test_com = self.get_batch_train_period_intubate(
            test_length, 0, data)
